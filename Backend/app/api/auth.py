...
def register():
    ...
    user = UserAuth(name=data["name"], email=data["email"],verified=data['verified'], password=data["password"])
    db.session.add(user)
    db.session.commit()
    return jsonify({"message": "User created successfully"}), 201

@app.route("/api/send_verification_otp", methods=["POST","GET"])
def send_verification_otp():
    ...
    <html>
    <body>
        <p>It is a verification mail. So please make sure verify before register</p>
        <h4>This is your verification code</h4>
        <h1>{otp}</h1>
    </body>
    </html>
    """
    send_email_html("mad2project.ticketshow@gmail.com",email, 'mail verification',body)
    return jsonify({"message": "verification code sent successfully","otp":otp}), 200
