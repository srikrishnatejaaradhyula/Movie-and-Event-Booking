...
const changePassword = async() => {
   ...
   if (confirm_password.value == ''){
    ...
    if (confirm_password.value.length < 6){
        toast.warning('Confirm Password must be at least 6 characters');
    }
    if(!validatePassword(password.value)){
        toast.warning('Password must contain at least one uppercase, one lowercase, one number and one special character');
    }
