    ...
    <div class="container ">
         <div class="row align-items-center mx-0 my-3 d-flex justify-content-center">
             <div class="show h-auto col-md-6 col-12 shadow-lg  rounded-4">
                 <div class="sho mb-3 ">
                    <h4 class="col-md-12 text-center ">Create Movie</h4>
                     <form>
                     <div class="row">
                        <div class="col-md-6">
...
const convertToEmbedLink = (youtubeLink)  => {
  ...
  var match = youtubeLink.match(youtubeRegExp);
...
const handleSubmit = async() => {
    ...
    if (name.value == '' || genre.value == '' || price.value == '' || rating.value == ''|| release_date.value == ''|| end_date.value == '' || cast.value == ''|| movie_trailer.value == ''|| type.value == ''|| file.value == null || description.value == '' || duration.value == '' || language.value == '' ) {
        ...
        reader.onload = async () => {
            ...
            try {
                const response = await fetch(`${config.url}/api/create_movie`, {
                    method: 'POST',
                    headers: {
                ...
                if (!response.ok) {
                    ...
                    toast.success('Movie Created Successfully!',{autoClose: 1000});
...
onUpdated(() => {
    ...
    for (let i = 0; i < movie.movies.length; i++) {
        if (name.value == movie.movies[i].name) {
    ...
    for (let i = 0; i < movie.movies.length; i++) {
        if (release_date.value <= movie.movies[i].end_date && release_date.value != '') {
            toast.warning('movie already exists on this date');
            release_date.value = '';
        }
    }
    for (let i = 0; i < movie.movies.length; i++) {
        if (end_date.value <= movie.movies[i].end_date && end_date.value != '') {
            toast.warning('movie already exists on this date');
            end_date.value = '';
        }
    }
