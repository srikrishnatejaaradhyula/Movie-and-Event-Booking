    ...
    <div class="container ">
         <div class="row align-items-center mx-0 my-3 d-flex justify-content-center">
             <div class="show h-auto col-md-6 col-12 shadow-lg  rounded-4">
                 <div class="sho mb-3 ">
                    <h4 class="col-md-12 text-center ">Update Event</h4>
                     <form>
                     <div class="row" >
                        <div class="col-md-6">
                            <div class="col-md-12 form-floating md-5 " v-if="fileUrl">
                                    <a :href="fileUrl" >Prview</a>
                                    <button class="btn btn-outline-secondary  h-auto w-auto" @click="fileUrl=null" type="button" >remove</button>
                            </div>
                            <div class="col-md-12 form-floating " v-else>
...
// const handleSubmit = () => {
//     if (name.value == ''  || price.value == '' || date.value == '' || description.value == '' || address.value == '' || location.value == '' || form.value == '' ) {
...
//             try {
//                 const response = await fetch(`${config.url}/api/update_event`, {
//                     method: 'POST',
//                     headers: {
    ...
    if (
        ...
        try {
            ...
            if (fileUrl.value == null) {
                if (file.value == null) {
                    toast.warning('Please upload event brochure');
                    return;
                }
                ...
                reader.onload = () => {
                    ...
                    // Make the fetch request with file and form data
                    makeFetchRequest(requestBody);
                };
            } else {
                // Make the fetch request without file and form data
                const requestBody = {
                    admin_id: user.id,
                    id: id.value,
                    name: name.value,
...
const makeFetchRequest = async (requestBody) => {
    try {
        const response = await fetch(`${config.url}/api/update_event`, {
            method: 'POST',
            headers: {
